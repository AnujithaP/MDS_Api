    ...
    public class BpmnService
    {
        ...
        public async Task DeployProcessDefinition()
        {
            var bpmnResourceStream = this.GetType()
                .Assembly
                .GetManifestResourceStream("HeroesForHire.Bpmn.hire-heroes.bpmn");      // TODO: Use our own bpnm
        ...
        public async Task<string> StartProcessFor(Person person)
        {
            var processParams = new StartProcessInstance()
                .SetVariable("personId", VariableValue.FromObject(person.UniktId.Value.ToString()))
                .SetVariable("efternamn", VariableValue.FromObject(person.Efternamn))
                .SetVariable("customerCode", VariableValue.FromObject(person.Förnamn));

            processParams.BusinessKey = person.UniktId.Value.ToString();
